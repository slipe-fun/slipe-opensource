	...
	return (
		<div className='w-full z-10 p-4 pr-0 flex items-end overflow-hidden gap-5 bg-gradient-to-t from-[#00000060] to-[#00000000]'>
			<div
				...
				className='relative flex text-white rounded-full ease-out duration-200 
          data-[isexpanded=false]:w-[3.125rem]
          data-[isexpanded=true]:w-[calc(100%-1rem)]
          data-[isexpanded=true]:p-[0.375rem]
          data-[isexpanded=true]:bg-[#1f1f1f]'
			>
				<QuickReactions
				...
				>
					<Svg
						data-isactive={isReactions}
						className='!w-7 !h-7 duration-200 ease-out data-[isactive=true]:!w-[1.625rem] data-[isactive=true]:!h-[1.625rem]'
						icon={icons["smile"]}
					/>
				</button>
				<ReactionsModal
				...
				<AnimatePresence>
					{localReactions.map((reaction, index) => (
						<motion.button
							...
							onClick={reaction.name?.startsWith("emoji_") ? () => {} : () => reactionClicked(reaction.name[0], reaction.name.slice(2))}
