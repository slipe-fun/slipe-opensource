...
export default function Header({ slide, discardChanges, confirmed }) {
	...
	return (
		<div className='w-full p-4 pt-[calc(1rem+var(--safe-area-inset-top))] gap-7 data-[shadowed=true]:opacity-40 duration-200 ease-out flex top-0 border-b-[1px] animate-[fadeIn_0.3s_ease-out] border-foreground/10 z-50 bg-navigation items-center fixed'>
			<Button
				onClick={() => (slide === 1 && confirmed ? setDiscard(true) : navigate(-1))}
				size='icon'
				variant='secondary'
			...
			<Modal className="flex flex-col" setOpen={setDiscard} open={discard}>
				...
				<div className='w-full gap-4 border-t-[1px] border-foreground/10 p-4 flex'>
					<Button onClick={() => setDiscard(false)} size='full' variant='secondary' className='font-medium h-11 min-h-11 text-[0.9375rem] rounded-full'>
						Cancel
					</Button>
					<Button onClick={() => {setDiscard(false); discardChanges()}} variant='deleting' size='full' className='font-medium h-11 min-h-11 text-[0.9375rem] rounded-full'>
						Discard
					</Button>
				</div>
			</Modal>
