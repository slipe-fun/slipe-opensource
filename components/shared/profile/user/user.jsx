		...
		<motion.div data-covering={isCovering} className='w-full data-[covering=true]:z-10 flex flex-col -mb-16 z-40 -translate-y-16 gap-1 px-4'>
			<div className='flex justify-between items-end'>
				{user.avatar ? (
					<motion.img
						...
						style={{ scale, opacity }}
						className='rounded-full z-40 data-[covering=true]:pointer-events-none w-[7.5rem] bg-background aspect-square border-[5px] border-background object-cover'
			...
			<div className='flex items-center gap-4'>
				<div className='flex flex-col w-full overflow-hidden'>
					<div className='whitespace-nowrap max-w-fit text-[1.375rem] leading-7 text-ellipsis font-medium overflow-hidden'>{user.nickname || user.username || "Anonymous"}</div>
					<span className='text-foreground/50 w-full'>@{user.username || "unknown"}</span>
				</div>
				{isModal ? (
					<Button onClick={subscribeUser} data-subscribed={isSubscribed} className='data-[subscribed=true]:!bg-foreground/[0.12] text-lg rounded-full px-6 min-h-[3.25rem] h-[3.25rem]'>{isSubscribed ? "Unfollow" : "Follow"}</Button>
				) : (
